...
`@heroicons/react` is the preferred icon library (already installed).
...
`react-icons` is also installed and may be used for icons not available in Heroicons (brand icons, specific tech icons).
...
- Never paste raw `<svg>` markup inline.
- Never import from Font Awesome, Lucide, or Material Icons packages.
- Always use Tailwind sizing classes (`w-* h-*`); never an inline `style`.
